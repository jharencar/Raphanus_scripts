...
#SBATCH -D /group/jrigrp11/juliagh/GEMMA/output/log_response_ratio_0.1geno 
...
# DOesn't work bc cna't load qqman, would need to figure out maybe a cond env in whhich I load qqman or somtheing...
module load R

# run the code - remember to first open the script and change paths to make sure it is making figures from the right files
Rscript /home/jgharenc/Raphanus_scripts/plot_GEMMA_output.R > $SLURM_JOBID.out
