...
#SBATCH -D /group/jrigrp11/juliagh/moe_data_deduped_trimmed/ 
...
#SBATCH -A jrigrp
#SBATCH -p high2
#SBATCH --time=3-12:00:00 #not sure actual time, this is based on old trimmomatic script
#SBATCH --mem=20G # no idea how much needed, nothing specified on previous script and no immediate info online                   
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# load modules
module load openjdk trimmomatic
...
R1=$(ls *clumped1.fq.gz| sed -n ${SLURM_ARRAY_TASK_ID}p)
R2=$(ls *clumped2.fq.gz| sed -n ${SLURM_ARRAY_TASK_ID}p)

# e.g. assigns "MB_P2_P2E11_S997" from MB_P2_P2E11_S997_clumped2.fq.gz as SAMPLE_NAME
SAMPLE_NAME=$(echo $R1 |cut  -d "_" -f 1,2,3,4 )
