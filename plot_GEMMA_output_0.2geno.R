...
plot_gemma_results <- function(gemma_output_file_2) {
  
  # Read the GEMMA output into a data frame
  gemma_data <- read.table(gemma_output_file_2, header = TRUE, stringsAsFactors = FALSE)
  
  # Extract the trait name from the filename for use as title
  filename <- basename(gemma_output_file_2)
  ...
  plot_filename <- file.path(gemma_output_dir_2, paste0(trait_name, "_gemma_FDR_manhattan.pdf"))
...
for (gemma_output_file_2 in gemma_output_files_2) {
  print(paste("Processing file:", gemma_output_file_2))
  plot_gemma_results(gemma_output_file_2)
}
...
for (gemma_output_file_2 in gemma_output_files_2) {
  print(paste("Processing file (qqman):", gemma_output_file_2))
  ...
  if (!all(c("chr", "ps", "rs", "p_wald") %in% colnames(gemma_data))) {
    print(paste("Skipping file due to missing columns:", gemma_output_file_2))
    next
  }
  ...
  pdf(file = file.path(gemma_output_dir_2, paste0(trait_name, "_gemma_qqman_manhattan.pdf")),
      width = 12, height = 6)
